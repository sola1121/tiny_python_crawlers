...
def create_image(url, directory, COUNTER):
    COUNTER.value += 1
    image_link = url
...
if __name__ == "__main__":
    ...
    while True:
        ...
        directory = DESKTOP + "animenylon_pl0%d\\" % HTML_NUM
        ...
        for url in page_lst:
            ...
            image_lst.append(res.get())

        pool.close()
        pool.join()

        for url in image_lst:
            if url is None:
                continue
            pro = Process(target=create_image, args=(url, directory, COUNTER))
